...
const GENERATOR_VERSION = "portfolio-card-v1";
...
async function validatePortrait(sourceBuffer, sourcePath) {
  ...
  if (!metadata.hasAlpha) {
    throw new Error(`Portrait must have a transparent background. The uploaded image does not contain an alpha channel.`);
  }

  const { channels } = await sharp(sourceBuffer).ensureAlpha().extractChannel("alpha").stats();
  if (channels[0].min === 255) {
    throw new Error(`Portrait must contain transparent pixels. Remove the background from the image before generating.`);
  }
}
...
async function cleanOldAssets(outputDirectory, currentFiles) {
  ...
  const generatedPattern = /^portfolio-card-[a-f0-9]{8}-(?:mobile-)?(?:dark|light)\.svg$/;
  await Promise.all(entries
    .filter((entry) => generatedPattern.test(entry) && !currentFiles.includes(entry))
    .map((entry) => unlink(resolve(outputDirectory, entry))));
}

export async function generateHeroAssets({ config, sourceBuffer, outputDirectory }) {
  await validatePortrait(sourceBuffer, "uploaded portrait");
  ...
  const assets = {
    desktopDark: `portfolio-card-${version}-dark.svg`,
    desktopLight: `portfolio-card-${version}-light.svg`,
    mobileDark: `portfolio-card-${version}-mobile-dark.svg`,
    mobileLight: `portfolio-card-${version}-mobile-light.svg`
  };
